...
/* ═══════════════════════════════════════════════════════════════════════════════════
   🍽️ EAT & PARK RESTAURANT — PROFESSIONAL POS V7.2 (ENHANCED EDITION)
   ✨ FULL MENU | OTP LOGIN | GET LOCATION | BOOKINGS | KITCHEN BOARD | THERMAL PRINT
   🎨 NEW: Micro-interactions, Smart Toasts, Accessibility, Analytics, Rate Limiting
═══════════════════════════════════════════════════════════════════════════════════ */

// ✨ ENHANCED FONTS & ANIMATIONS
const FONTS = `
...
.slide-from-right { animation: slideInFromRight 0.35s cubic-bezier(0.16, 1, 0.3, 1); }
...
@media (prefers-reduced-motion: reduce) {
  .pulse-anim, .scale-bounce, .smooth-transition, .smooth-slide-up, .fade-scale, .skeleton-loading, .success-pulse, .slide-from-right { animation: none !important; transition: none !important; }
}
...
const EMPTY_STATES = {
  veg_filtered: {
    title: "🥬 No vegetarian options here",
    subtitle: "Try 'All Items' or check our Paneer & Mushroom section!",
  },
  search_no_results: {
    title: "🔍 Dish not found",
    subtitle: "Try searching 'paneer', 'chicken', 'biryani', or 'tandoori'",
  },
  category_empty: {
    title: "📭 This category is empty",
    subtitle: "Check out our bestsellers in Fun Food or Tandoori!",
  },
};
...
const toastPriority = {
  ...
  warning: { duration: 4000, bg: '#ff9800', icon: '⚠️' }
};

// ✨ RATE LIMITER CLASS (Security)
class RateLimiter {
  constructor() {
    this.attempts = {};
  }
...
function VegDot({ veg }) { 
  ...
  return (
    <span 
      role="img"
      aria-label={veg ? "Vegetarian item" : "Non-vegetarian item"}
...
function SearchBar({ value, onChange, placeholder = "Search menu..." }) {
  ...
  return (
    <div style={{ position: "relative", flex: 1 }}>
      <input type="text" value={value} onChange={handleSearch} placeholder={placeholder} style={{...inputStyle, paddingLeft: 42, background: "#fff"}} onFocus={(e) => { e.target.style.borderColor = COLORS.copper; }} onBlur={(e) => { e.target.style.borderColor = COLORS.line; }} />
...
function MenuSkeleton() {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", padding: "16px 0", gap: 12 }}>
      <div style={{ flex: 1 }}>
        <div className="skeleton-loading" style={{ height: 20, marginBottom: 8, borderRadius: 8 }} />
        <div className="skeleton-loading" style={{ height: 16, marginBottom: 8, borderRadius: 8, width: '80%' }} />
        <div className="skeleton-loading" style={{ height: 14, borderRadius: 8, width: '60%' }} />
      </div>
...
// ✨ ENHANCED TOAST WITH PRIORITY SYSTEM
function Toast({ message, type = 'info' }) {
  ...
  return (
    <div className="toast-anim" style={{ 
      ...
      padding: '16px 28px', 
      borderRadius: 30, 
      boxShadow: `0 12px 28px ${config.bg}40`, 
      zIndex: 100, 
      fontWeight: 700, 
      fontSize: 15, 
      display: 'flex', 
...
// ✨ ORDER ANALYTICS COMPONENT (New)
function MyOrderStats({ myOrders, currentCoins }) {
  if (myOrders.length === 0) return null;

  const totalSpent = myOrders.reduce((s, o) => 
    s + o.items.reduce((a, i) => a + i.price * i.qty, 0), 0);
  ...
  const favoriteDish = (() => {
    ...
    const entries = Object.entries(dishes).sort((a, b) => b[1] - a[1]);
    return entries.length > 0 ? entries[0][0] : 'Unknown';
  })();

  return (
    <div style={{ background: 'linear-gradient(135deg, #fdfbfb 0%, #ebedee 100%)', padding: 16, borderRadius: 12, marginBottom: 16, border: `1px solid ${COLORS.line}` }}>
      <h4 style={{ margin: '0 0 16px', color: COLORS.ink, fontWeight: 800 }}>📊 Your Journey</h4>
      <div style={{ display: 'grid', gap: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: 12, background: '#fff', borderRadius: 10 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: 12, background: '#fff', borderRadius: 10 }}>
          ...
          <strong style={{ color: COLORS.ink }}>{favoriteDish}</strong>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: 12, background: '#fff', borderRadius: 10 }}>
...
function CustomerView({ menu, orders, placeOrder, bookEvent, gallery, offersList, table, setTable, setRole, promo, settings, installApp, handlePrint, isDark, setIsDark, requestWaiter, loyaltyRules, loyaltyUsers }) {
  ...
  const handleSetQty = (id, q) => {
    ...
    if (q > oldQ && q === 1) {
      ...
      if(addedItem) {
        ...
        if (hour < 11) {
          suggestionPool = menu.filter(m => m.category.includes("Tea") || m.category.includes("Bread"));
        } else if (hour < 13) {
          suggestionPool = menu.filter(m => m.category.includes("Biryani") || m.category.includes("Pulao"));
        } else if (hour < 17) {
          suggestionPool = menu.filter(m => m.category.includes("Snacks") || m.category.includes("Drink"));
        } else {
          suggestionPool = menu.filter(m => m.category.includes("Tandoori") || m.category.includes("Mains"));
        }
  ...
  // ✨ ENHANCED TOAST WITH PRIORITY & HAPTIC FEEDBACK
  const showToast = (msg, type = 'info') => {
    const config = toastPriority[type] || toastPriority.info;
    
    if (config.sound && navigator.vibrate) {
  ...
  const handleSendOtp = () => {
    ...
    if (!check.allowed) {
      showToast(`⏱️ Too many attempts. Try again in ${check.retryAfter}m`, 'warning');
      return;
    }
  ...
  const handleApplyCoupon = () => {
    ...
    if (!coupon.condition()) {
      if (code === "WELCOME20") {
        showToast("💳 Welcome coupon valid for first order only", 'warning');
      } else if (code === "LOYALTY50") {
        showToast("👑 VIP coupon requires 500+ EatCoins", 'warning');
      }
      return;
    }
      ...
      <div style={{ padding: "16px" }}>
        ...
        {filteredItems.length === 0 && (
          <div style={{ textAlign: 'center', padding: '40px 20px', color: COLORS.textLight }}>
            ...
            <div style={{ fontWeight: 700, fontSize: 15, color: COLORS.ink, marginBottom: 8 }}>
              {vegOnly ? "No vegetarian options" : searchQuery ? "Nothing found" : "Category empty"}
            </div>
            <div style={{ fontWeight: 500, fontSize: 14, lineHeight: 1.6 }}>
              {vegOnly 
                ? "Try 'All Items' filter or explore Paneer & Mushroom section"
                : searchQuery 
                ? "Try searching 'paneer', 'chicken', 'biryani', or 'tandoori'"
                : "Browse bestsellers in Fun Food or Tandoori!"}
            </div>
          </div>
        )}
          ...
          <div key={item.id} style={{ display: "flex", justifyContent: "space-between", padding: "16px 0", borderBottom: `1px solid ${COLORS.line}`, gap: 12, opacity: item.available ? 1 : 0.6 }} className="slide-up">
            ...
            <div style={{ position: "relative", width: 90, height: 90, flexShrink: 0 }}>
              <img 
                ...
                style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 14, filter: item.available ? 'none' : 'grayscale(100%)', backfaceVisibility: 'hidden' }} 
              />
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 80, display: "flex" }}>
          <div style={{ width: "80%", maxWidth: 300, background: "#fff", height: "100%", padding: "24px", display: "flex", flexDirection: "column", boxShadow: "4px 0 30px rgba(0,0,0,0.2)", overflowY: "auto" }} className="slide-right">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12, flex: 1 }}>
              ...
              <SidebarBtn icon="📊" text="My Stats" onClick={() => {setShowSidebar(false); setActiveModal('stats');}} />
      ...
      {(cartOpen || activeModal) && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "flex-end", zIndex: 70 }} onClick={() => {setCartOpen(false); setActiveModal(null); setConfirmedBooking(null);}}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", width: "100%", maxWidth: 480, margin: "0 auto", borderRadius: "24px 24px 0 0", padding: "24px 20px 30px", maxHeight: "85vh", overflowY: "auto", boxShadow: "0 -10px 40px rgba(0,0,0,0.15)" }} className="slide-up">
            ...
            {activeModal === 'stats' && (
              <>
                <ModalHeader title="📊 Your Stats" onClose={() => setActiveModal(null)} />
                <MyOrderStats myOrders={myActiveOrders} currentCoins={currentCoins} />
              </>
            )}
...
function StaffView({ orders, advanceStatus, setRole, calls, resolveCall }) {
  ...
  return (
    <div style={{ padding: "26px 20px 60px", maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div style={{ fontSize: 15, color: COLORS.textLight, marginBottom: 24, fontWeight: 600 }}>Use ↑↓ to navigate, P/R to advance | Press ? for help</div>
      ...
      {showHelpModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }} onClick={() => setShowHelpModal(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', padding: 28, borderRadius: 20, width: '90%', maxWidth: 500, maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 20px 50px rgba(0,0,0,0.3)' }} className="slide-up">
            <h2 style={{ margin: '0 0 24px', fontFamily: "'Outfit', sans-serif" }}>⌨️ Keyboard Shortcuts</h2>
            <div style={{ display: 'grid', gap: 12 }}>
              {Object.entries(SHORTCUTS).map(([key, desc]) => (
                <div key={key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: COLORS.paper, borderRadius: 10 }}>
                  <kbd style={{ background: COLORS.copper, color: '#fff', padding: '4px 10px', borderRadius: 6, fontWeight: 800, fontFamily: 'monospace' }}>{key}</kbd>
                  <span style={{ fontWeight: 600 }}>{desc}</span>
                </div>
              ))}
            </div>
            <button onClick={() => setShowHelpModal(false)} style={{ width: '100%', marginTop: 20, ...primaryBtn }}>Close</button>
          </div>
        </div>
      )}
    </div>
...
function AdminView({ menu, setMenuState, bookings, orders, markPaid, deleteOrder, setRole, inventory, addInventory, updateStock, deleteBooking, offersList, addOffer, removeOffer, loyaltyRules, setLoyaltyRules, loyaltyUsers }) {
  ...
  const getMetrics = () => {
    const validOrders = filteredOrders.filter(o => o.status === "served");
    if (validOrders.length === 0) return null;
    return {
      avgPrepTime: (validOrders.length > 0 ? 15 : 0).toFixed(1),
      ...
      {tab === "overview" && (
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20, marginBottom: 28 }}>
            ...
            {metrics && <StatCard label="Avg Prep Time" value={`${metrics.avgPrepTime}m`} icon="⏱️" color={COLORS.copper} />}
          </div>
        </>
      )}
