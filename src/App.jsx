...
function CustomerView({ menu, orders, placeOrder, bookEvent, gallery, offersList, table, setTable, requestPinPrompt, settings, isDark, setIsDark, requestWaiter, loyaltyRules, loyaltyUsers, coinHistory, setOrdersState }) {
  ...
  useEffect(() => {
    if (custName || custPhone) {
      localStorage.setItem('eatpark_customer', JSON.stringify({ name: custName, phone: custPhone, address: custAddress, isLoggedIn }));
    }
    ...
    try {
      ...
      const order = {
        ...
        items: cartItems.map(([id, qty]) => {
          ...
          return { itemId: id, name: m.name, portion: m.portion || "", price: m.price, qty };
        }),
      ...
      try {
        const userRef = doc(db, "customers", custPhone);
        await updateDoc(userRef, { totalOrders: customerData.totalOrders, totalSpent: customerData.totalSpent, lastOrderDate: Date.now() });
      } catch (e) {
...
export default function App() {
  ...
  const placeOrder = async (order, rewardCost = 0) => {
    ...
    if (order.customer.phone && order.customer.phone.length >= 10) {
      ...
      if (existingUserIndex >= 0) { try { await setDoc(doc(db, "loyaltyUsers", order.customer.phone), updatedUsers[existingUserIndex]); } catch (e) { } } else { try { await setDoc(doc(db, "loyaltyUsers", order.customer.phone), { phone: order.customer.phone, name: order.customer.name, coins: netCoins }); } catch (e) { } }
