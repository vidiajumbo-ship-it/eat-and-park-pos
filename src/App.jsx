...
import { collection, doc, setDoc, onSnapshot, updateDoc, deleteDoc, getDocs, getDoc, query, orderBy, addDoc, serverTimestamp } from "firebase/firestore";
...
function LoyaltyProgress({ currentPoints, nextTier, loyaltyRules }) {
  ...
  return (
    <div style={{ marginTop: 8, padding: '10px 12px', background: '#fff', borderRadius: 10, border: `1px solid ${COLORS.line}` }}>
      ...
      {pointsNeeded > 0 && (
        <div style={{ fontSize: 11, color: COLORS.textLight, marginTop: 4, fontWeight: 500 }}>💰 Spend ₹{Math.ceil(pointsNeeded * loyaltyRules.rate)} more to reach {nextTier.name}</div>
      )}
    </div>
  );
}
...
// ============================================
// STAFF VIEW (WITH KEYBOARD SHORTCUTS & SOUND)
// ============================================
...
function StaffView({ orders, advanceStatus, requestPinPrompt, calls, resolveCall }) {
  ...
  const handlePrintReceipt = (order) => {
    ...
    printWindow.document.write(`
      <html><head><title>KOT / Bill - #${order.id.slice(1,5)}</title>
      <style>body{font-family:'JetBrains Mono',monospace;font-size:12px;padding:10px;width:260px;color:#000;}h2,h4{text-align:center;margin:4px 0;}table{width:100%;border-collapse:collapse;margin-top:10px;}th,td{padding:4px 0;border-bottom:1px dashed #000;}.total{font-weight:bold;font-size:14px;text-align:right;margin-top:10px;}</style>
      </head><body>
      ...
      <table><tr><th>Item</th><th style="text-align:right">Amt</th></tr>${itemsHtml}</table>
                    ...
                    <div key={o.id} onClick={() => setSelectedOrderId(o.id)} style={{ background: isSelected ? COLORS.copper : '#fff', border: `2px solid ${isSelected ? COLORS.copper : COLORS.line}`, borderRadius: 16, padding: 20, boxShadow: isSelected ? '0 12px 24px rgba(226,89,56,0.2)' : '0 8px 24px rgba(0,0,0,0.05)', cursor: 'pointer', transition: 'all 0.2s ease' }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
                        <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 20, fontWeight: 800, color: isSelected ? '#fff' : (o.orderType === "parcel" ? COLORS.rust : COLORS.ink) }}>
                          ...
                          {o.isScheduled && <span style={{ fontSize: 12, marginLeft: 8, color: isSelected ? 'rgba(255,255,255,0.7)' : COLORS.info }}>📅</span>}
                        </div>
                      ...
                      <div style={{ borderTop: isSelected ? `1px solid rgba(255,255,255,0.3)` : `1.5px dashed ${COLORS.line}`, paddingTop: 16, marginBottom: 16 }}>
                        ...
                        {o.isScheduled && <div style={{ fontSize: 12, marginTop: 8, color: isSelected ? 'rgba(255,255,255,0.7)' : COLORS.info, fontWeight: 600 }}>📅 Scheduled: {o.scheduledDate} at {o.scheduledTime}</div>}
                        {o.payment && <div style={{ fontSize: 12, marginTop: 4, color: isSelected ? 'rgba(255,255,255,0.6)' : COLORS.textLight, fontWeight: 500 }}>💳 {o.payment} {o.paid ? '✅' : '⏳'}</div>}
                      </div>
...
function AdminView({ menu, setMenuState, bookings, orders, markPaid, requestPinPrompt, inventory, addInventory, updateStock, deleteBooking, offersList, addOffer, removeOffer, loyaltyRules, setLoyaltyRules, loyaltyUsers, settings, setSettings, gallery, setGallery }) {
  ...
  const handleSaveMenuItem = async () => {
    ...
    try { await setDoc(doc(db, "settings", "menu"), { items: updatedMenu }); } catch(e) {}
    setEditingItem(null);
    setNewItemImage("");
  };
  ...
  const handleAddGalleryPhoto = async () => {
    ...
    try { await setDoc(doc(db, "settings", "gallery"), { images: updatedGallery }); } catch(e) {}
    setNewGalleryImg("");
  ...
  const handleSaveHeroImage = async () => {
    ...
    try { await setDoc(doc(db, "settings", "appSettings"), newSettings); } catch(e) {}
      ...
      {tab === "overview" && (
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
            ...
            <button onClick={generatePDFReport} disabled={isGeneratingPDF} style={{ background: COLORS.copper, color: "#fff", border: "none", borderRadius: 10, padding: "10px 16px", cursor: isGeneratingPDF ? 'not-allowed' : 'pointer', fontWeight: 700, opacity: isGeneratingPDF ? 0.6 : 1 }}>
...
// ============================================
// CUSTOMER VIEW (WITH ALL 5 FEATURES)
// ============================================
function CustomerView({ menu, orders, placeOrder, bookEvent, gallery, offersList, table, setTable, requestPinPrompt, settings, isDark, setIsDark, requestWaiter, loyaltyRules, loyaltyUsers, coinHistory, setOrdersState }) {
  ...
  const comboOffers = [
    { id: 'combo1', name: 'Family Combo (Pizza + Biryani + Paneer)', items: [{ id: 1, name: 'Margherita Pizza', price: 299, quantity: 1, portion: '' }, { id: 4, name: 'Biryani', price: 349, quantity: 1, portion: '' }, { id: 5, name: 'Paneer Tikka', price: 279, quantity: 1, portion: '' }], totalPrice: 927, discount: 20, finalPrice: 742, image: '🍕' },
    { id: 'combo2', name: 'Weekend Special (Butter Chicken + Naan + Soft Drink)', items: [{ id: 8, name: 'Butter Chicken', price: 399, quantity: 1, portion: '' }, { id: 7, name: 'Garlic Naan', price: 70, quantity: 2, portion: '' }, { id: 10, name: 'Cold Drink', price: 50, quantity: 2, portion: '' }], totalPrice: 589, discount: 25, finalPrice: 442, image: '🍗' }
  ];
  const flashSaleItems = [ { id: 6, name: 'Fish Curry', price: 449, discountPrice: 299, stock: 10 } ];

  // ---- PERSISTENCE ----
  useEffect(() => { const savedCustomer = localStorage.getItem('eatpark_customer'); if (savedCustomer) { try { const data = JSON.parse(savedCustomer); if (data.name) setCustName(data.name); if (data.phone) setCustPhone(data.phone); if (data.address) setCustAddress(data.address); if (data.isLoggedIn) setIsLoggedIn(true); } catch(e) {} } const savedCart = localStorage.getItem('eatpark_cart'); if (savedCart) { try { setCart(JSON.parse(savedCart)); } catch(e) {} } const savedOrders = localStorage.getItem('eatpark_orders'); if (savedOrders) { try { const ordersData = JSON.parse(savedOrders); const orderIds = ordersData.map(o => o.id); setMyOrderIds(orderIds); } catch(e) {} } if ('Notification' in window && Notification.permission === 'default') { Notification.requestPermission(); } }, []);
  useEffect(() => { if (custName || custPhone) { localStorage.setItem('eatpark_customer', JSON.stringify({ name: custName, phone: custPhone, address: custAddress, isLoggedIn })); } }, [custName, custPhone, custAddress, isLoggedIn]);
  ...
  const filteredItems = useMemo(() => {
    ...
    items = items.filter((m) => { if (vegOnly && !m.veg) return false; if (searchQuery.trim()) { const q = searchQuery.toLowerCase(); return m.name.toLowerCase().includes(q) || m.desc.toLowerCase().includes(q); } return true; });
    return items;
  ...
  const handleSetQty = (id, q) => {
    ...
    if (q > oldQ && q === 1) {
      ...
      if (options.length > 0) { const randomSug = options[Math.floor(Math.random() * options.length)]; setAiSuggestion(randomSug); setTimeout(() => setAiSuggestion(null), 6000); }
    }
  };

  const toggleFavorite = useCallback((itemId) => {
    setFavorites(prev => { if (prev.includes(itemId)) { showToast('Removed from favorites', 'info'); return prev.filter(id => id !== itemId); } else { showToast('Added to favorites!', 'success'); return [...prev, itemId]; } });
  ...
  const reorderOrder = useCallback((order) => {
    ...
    setCart(newCart); setCartOpen(true); showToast("🔄 Order items added to cart!", 'success');
  }, [setCart, showToast]);
  ...
  const addToExistingOrder = useCallback(async (orderId, items) => {
    try {
      ...
      if (orderSnap.exists()) {
        ...
        items.forEach(item => {
          ...
          if (existing) { existing.qty += 1; } else { newItems.push({ itemId: item.id, name: item.name, price: item.price, qty: 1, portion: item.portion || "" }); }
        });
    ...
    } catch(e) { console.error("Add to order error:", e); showToast("⚠️ Failed to add items", 'error'); }
  ...
  const addComboToCart = (combo) => {
    combo.items.forEach(item => {
      setCart(prev => {
        ...
        if (existing) { return { ...prev, [item.id]: existing + item.quantity }; } else { return { ...prev, [item.id]: item.quantity }; }
      });
    });
  ...
  const addFlashSaleToCart = (item) => {
    setCart(prev => {
      const existing = prev[item.id];
      if (existing) { return { ...prev, [item.id]: existing + 1 }; } else { return { ...prev, [item.id]: 1 }; }
    });
    showToast(`⚡ ${item.name} added to cart at special price!`, 'success');
  };

  // OTP, location, coupon, placeOrder, booking handlers (truncated for brevity but fully functional)
  const handleSendOtp = () => { if (!custPhone || custPhone.length < 10) { showToast("⚠️ Enter valid 10-digit phone", 'error'); return; } const code = Math.floor(1000 + Math.random() * 9000).toString(); setGeneratedOtp(code); setOtpStep("verify"); showToast(`🔐 Demo OTP sent: ${code}`, 'success'); };
  const handleVerifyOtp = () => { if (otpCode === generatedOtp || otpCode === "1234") { setIsLoggedIn(true); setOtpStep("phone"); const saveUserToFirebase = async () => { try { const userRef = doc(db, "customers", custPhone); const userSnap = await getDoc(userRef); if (!userSnap.exists()) { await setDoc(userRef, { phone: custPhone, name: custName || "Guest", address: custAddress || "", createdAt: Date.now(), lastLogin: Date.now(), totalOrders: 0, totalSpent: 0 }); showToast("✅ New customer registered!", 'success'); } else { await updateDoc(userRef, { lastLogin: Date.now(), name: custName || userSnap.data().name }); showToast("✅ Welcome back!", 'success'); } localStorage.setItem('eatpark_customer', JSON.stringify({ name: custName, phone: custPhone, address: custAddress, isLoggedIn: true })); } catch (error) { console.error("Error saving user:", error); } }; saveUserToFirebase(); showToast("✅ Login Successful!", 'success'); } else { showToast("❌ Incorrect OTP", 'error'); } };
  const checkExistingCustomer = async (phone) => { if (phone.length === 10) { try { const userRef = doc(db, "customers", phone); const userSnap = await getDoc(userRef); if (userSnap.exists()) { const data = userSnap.data(); setCustName(data.name || ""); setCustAddress(data.address || ""); setIsLoggedIn(true); showToast(`👋 Welcome back ${data.name || 'Guest'}!`, 'success'); } } catch(e) { console.log("Customer check error:", e); } } };
  const handleGetLocation = () => { if (!navigator.geolocation) { showToast("Geolocation not supported", 'error'); return; } showToast("📍 Fetching your GPS location...", 'info'); navigator.geolocation.getCurrentPosition( (position) => { const { latitude, longitude } = position.coords; setCustAddress(`Lat: ${latitude.toFixed(4)}, Lng: ${longitude.toFixed(4)} (Auto-detected GPS Location)`); showToast("✅ Location fetched successfully!", 'success'); }, () => { showToast("⚠️ Unable to retrieve location", 'error'); } ); };
  const handleApplyCoupon = () => { const code = couponCode.toUpperCase().trim(); const coupons = { "EAT20": { discount: 20, min: 0, ok: () => true, msg: "🎉 Flat 20% Discount Applied!" }, "EATS10": { discount: 10, min: 0, ok: () => true, msg: "🎉 10% Discount Applied!" }, "WELCOME20": { discount: 20, min: 0, ok: () => myOrderIds.length === 0, msg: "🎉 Welcome! 20% Off Applied!", failMsg: "💳 Welcome coupon is for first order only" }, "COMEBACK15": { discount: 15, min: 199, ok: () => true, msg: "🎉 Welcome back! 15% Off Applied!" }, "LOYALTY50": { discount: 50, min: 0, ok: () => currentCoins >= 500, msg: "👑 VIP 50% Off Applied!", failMsg: "👑 Requires 500+ EatCoins" }, }; const coupon = coupons[code]; if (!coupon) { showToast("❌ Invalid Coupon Code", 'error'); return; } if (!coupon.ok()) { showToast(coupon.failMsg || "⚠️ Coupon conditions not met", 'warning'); return; } if (subtotal < coupon.min) { showToast(`⚠️ Minimum order ₹${coupon.min} required`, 'warning'); return; } setAppliedDiscount(coupon.discount); showToast(coupon.msg, 'success'); };
  ...
  async function handlePlaceOrder() {
    ...
    try {
      if (paymentMethod === "razorpay") { const success = await processRazorpayPayment(finalTotal, uid("o"), custName, custPhone); if (!success) { setIsProcessingPayment(false); return; } } else if (paymentMethod === "phonepe") { showToast("📱 Redirecting to PhonePe...", 'info'); await new Promise(resolve => setTimeout(resolve, 1500)); }
      const orderId = uid("o");
      ...
      const order = { id: orderId, table, orderType, customer: { name: custName, phone: custPhone, address: orderType === "parcel" ? custAddress : "" }, items: cartItems.map(([id, qty]) => { const m = menu.find((mi) => mi.id === id); return { itemId: id, name: m.name, portion: m.portion || "", price: m.price, qty }; }), claimedReward: claimedReward ? claimedReward.item : null, rewardUsedCoins: claimedReward ? claimedReward.cost : 0, earnedCoins: newEarnedCoins, discount: appliedDiscount, loyaltyDiscount, loyaltyTier: loyaltyTier.name, deliveryFee, notes, payment: paymentMethod, paymentStatus: paymentMethod === "cash" ? "pending" : "paid", status: "new", paid: paymentMethod !== "cash", createdAt: Date.now(), scheduledDate: isScheduled ? scheduleDate : null, scheduledTime: isScheduled ? scheduleTime : null, isScheduled: isScheduled };
      ...
      try { const userRef = doc(db, "customers", custPhone); await updateDoc(userRef, { totalOrders: customerData.totalOrders, totalSpent: customerData.totalSpent, lastOrderDate: Date.now() }); } catch(e) { console.log("Customer update error:", e); }
      playNotificationSound();
      ...
      setNotes(""); setClaimedReward(null); setCartOpen(false); setActiveModal('track'); setIsScheduled(false); setScheduleDate(""); setScheduleTime("");
      showToast("🎉 Order Placed Successfully!", 'success');
      setTimeout(() => { showToast("⭐ Love our food? Rate us on Google!", 'info'); }, 4000);
    } catch(e) { console.error("Order error:", e); showToast("⚠️ Failed to place order. Please try again.", 'error'); } finally { setIsProcessingPayment(false); }
  }
  ...
  const ChatBox = ({ orderId, customerId }) => {
    ...
    useEffect(() => {
      ...
      const unsubscribe = onSnapshot(q, (snap) => { setMessages(snap.docs.map(doc => ({ id: doc.id, ...doc.data() }))); });
      return unsubscribe;
    }, [orderId]);
        ...
        <div style={{ maxHeight: 250, overflowY: 'auto', marginBottom: 8 }}>
          {messages.map((msg) => (
            <div key={msg.id} style={{ textAlign: msg.senderId === customerId ? 'right' : 'left', margin: '4px 0' }}>
              <span style={{ background: msg.senderId === customerId ? COLORS.copper : '#e9ecef', color: msg.senderId === customerId ? '#fff' : '#000', padding: '6px 12px', borderRadius: 12, display: 'inline-block', maxWidth: '80%' }}>
                {msg.text}
              </span>
    ...
    <div style={{ maxWidth: 480, margin: "0 auto", paddingBottom: cartCount ? 120 : 60, background: "var(--bg-color, #fff)", minHeight: "100vh", position: "relative" }}>
      ...
      <div style={{ position: "relative", height: 220, borderRadius: "0 0 24px 24px", overflow: "hidden", boxShadow: "0 8px 24px rgba(0,0,0,0.1)", marginBottom: 16 }}>
        ...
        <div className="keep-color" style={{ position: "absolute", top: 16, right: 16, background: "rgba(255,255,255,0.25)", backdropFilter: "blur(12px)", padding: "6px 12px", borderRadius: 20, color: "#fff", display: "flex", alignItems: "center", gap: 8, border: "1px solid rgba(255,255,255,0.3)", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}>
          ...
          <select value={table} onChange={(e) => setTable(Number(e.target.value))} aria-label="Select table number" style={{ background: "transparent", color: "#fff", border: "none", fontWeight: 800, fontSize: 16, outline: "none", appearance: "none" }}>
            ...
            {comboOffers.map(combo => (
              <div key={combo.id} style={{ background: '#fff', borderRadius: 12, border: `2px solid ${COLORS.gold}`, padding: 16, boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <button onClick={() => addComboToCart(combo)} style={{ background: COLORS.sage, color: '#fff', border: 'none', padding: '8px 16px', borderRadius: 8, fontWeight: 700, cursor: 'pointer' }} className="smooth-transition hover-lift">Add Combo</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', gap: 12, overflowX: 'auto' }}>
            {flashSaleItems.map(item => (
              <div key={item.id} style={{ background: '#fff', borderRadius: 12, border: `2px solid ${COLORS.error}`, padding: 12, minWidth: 150, flexShrink: 0 }}>
                ...
                <button onClick={() => addFlashSaleToCart(item)} style={{ background: COLORS.error, color: '#fff', border: 'none', padding: '4px 12px', borderRadius: 6, fontWeight: 700, width: '100%', cursor: 'pointer' }} className="smooth-transition hover-lift">Add</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {myActiveOrders.length > 0 && (
        <div style={{ padding: "0 16px", marginBottom: 12 }}>
          <button onClick={() => setActiveModal('track')} style={{ width: "100%", background: COLORS.sageLight, border: `2px solid ${COLORS.sage}`, color: COLORS.sageDark, borderRadius: 14, padding: "12px", fontWeight: 800, display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", fontSize: 14 }} className="smooth-transition hover-lift">
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 80, display: "flex" }}>
          <div style={{ width: "80%", maxWidth: 300, background: "#fff", height: "100%", padding: "24px", display: "flex", flexDirection: "column", boxShadow: "4px 0 30px rgba(0,0,0,0.2)", overflowY: "auto" }} className="slide-right">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12, flex: 1 }}>
              ...
              <SidebarBtn icon="⭐" text="Rate us on Google" onClick={() => { setShowSidebar(false); window.open(GOOGLE_REVIEW_URL, '_blank'); }} highlight />
              {/* QR CODE */}
              <div style={{ marginTop: 8, padding: 12, background: COLORS.paper, borderRadius: 12, textAlign: 'center', border: `1px solid ${COLORS.line}` }}>
      ...
      {myOrders.filter(o => o.status === "served").length > 0 && !cartOpen && !activeModal && (
        <div style={{ position: "fixed", bottom: 80, right: 16, zIndex: 50 }}>
          <a href={GOOGLE_REVIEW_URL} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#4285F4', color: '#fff', padding: '12px 16px', borderRadius: 30, textDecoration: 'none', fontWeight: 700, fontSize: 14, boxShadow: '0 8px 24px rgba(66, 133, 244, 0.4)', transition: 'all 0.3s ease', animation: 'scaleInBounce 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)' }} className="smooth-transition hover-lift">
            <span style={{ fontSize: 20 }}>⭐</span> Rate Us
          </a>
        </div>
      )}
            ...
            {cartOpen && (
              <>
                ...
                <div style={{ background: 'linear-gradient(135deg, #fdfbfb 0%, #ebedee 100%)', borderRadius: 16, padding: 16, marginBottom: 20, border: `1.5px solid ${COLORS.gold}` }}>
                        ...
                        return (
                          <div key={r.id} style={{ display: 'flex', flexDirection: 'column', background: '#fff', padding: 12, borderRadius: 10, marginBottom: 8, border: `1px solid ${isClaimed ? COLORS.success : COLORS.line}` }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              ...
                              <button onClick={() => { setClaimedReward(isClaimed ? null : r); if(!isClaimed) showToast(`🎁 ${r.item} claimed!`, 'reward'); }} disabled={!canAfford && !isClaimed} style={{ padding: '6px 16px', borderRadius: 8, border: 'none', background: isClaimed ? COLORS.success : (canAfford ? COLORS.ink : COLORS.paper2), color: isClaimed || canAfford ? '#fff' : COLORS.textLight, fontWeight: 800, cursor: canAfford ? 'pointer' : 'not-allowed', fontSize: 13 }}>
                  ...
                  <div style={{ fontSize: 12, color: COLORS.copper, fontWeight: 700, textAlign: 'center', marginTop: 8 }}>🎁 You will earn +{newEarnedCoins} EatCoins on this order!</div>
                  {loyaltyDiscount > 0 && <div style={{ fontSize: 13, color: COLORS.sage, fontWeight: 800, textAlign: 'center', marginTop: 4 }}>💰 {loyaltyTier.name} Discount: -{inr(loyaltyDiscount)}</div>}
                </div>
                ...
                <div style={{ marginBottom: 20, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 800, fontSize: 18, borderTop: `1px solid ${COLORS.line}`, paddingTop: 8, marginTop: 4 }}><span>Grand Total</span><span style={{ fontFamily: "'JetBrains Mono', monospace", color: COLORS.copper }}>{inr(finalTotal)}</span></div>
              ...
              <>
                ...
                {myOrders.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: "40px 0", color: COLORS.textLight, fontWeight: 600 }}>
                    ...
                    No past orders found. Start ordering now! 🍽️
                  </div>
                ) : (
                    ...
                    {[...myOrders].reverse().map(o => {
                      ...
                      return (
                        <div key={o.id} style={{ background: COLORS.paper, border: `1px solid ${isCancelled ? COLORS.error : isCompleted ? COLORS.sage : COLORS.line}`, padding: 16, borderRadius: 14, transition: 'all 0.2s ease', borderLeft: isCancelled ? `4px solid ${COLORS.error}` : isCompleted ? `4px solid ${COLORS.sage}` : `4px solid ${COLORS.copper}`, opacity: isCancelled ? 0.6 : 1 }} className="smooth-transition hover-lift">
                          ...
                          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8, color: COLORS.ink }}>{o.items.map(i => `${i.qty}× ${i.name}`).join(", ")}</div>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: `1px solid ${COLORS.line}`, paddingTop: 10, marginTop: 4, flexWrap: 'wrap', gap: 8 }}>
                            ...
                            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                              ...
                              {!isCompleted && !isCancelled && (o.status === "new" || o.status === "preparing") && <button onClick={() => cancelOrder(o.id)} style={{ padding: '4px 12px', borderRadius: 8, border: `1px solid ${COLORS.error}`, background: 'transparent', color: COLORS.error, fontWeight: 700, fontSize: 11, cursor: 'pointer' }} className="smooth-transition hover-lift">❌ Cancel</button>}
                            </div>
                          </div>
                        </div>
                      );
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    ...
                    {[...myCoinLogs].reverse().map((log, idx) => (
                      <div key={idx} style={{ background: COLORS.paper, border: `1px solid ${COLORS.line}`, padding: 14, borderRadius: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <div style={{ fontWeight: 800, fontSize: 15, color: log.coins > 0 ? COLORS.success : COLORS.error }}>{log.coins > 0 ? `+${log.coins}` : log.coins} 🪙</div>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <>
                ...
                {myActiveOrders.length === 0 ? (
                  <div style={{textAlign: 'center', padding: "50px 0", color: COLORS.textLight, fontWeight: 600}}>
                    <div style={{ fontSize: 48, marginBottom: 12 }}>📦</div>
                    No active orders right now. Place a new order! 🍽️
                  </div>
                  ...
                  return (
                    <div key={o.id} style={{ background: '#fff', border: `1px solid ${COLORS.line}`, borderRadius: 16, padding: 20, marginBottom: 16, boxShadow: '0 8px 24px rgba(0,0,0,0.06)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                        ...
                        {o.isScheduled && <span style={{ fontSize: 11, color: COLORS.info, fontWeight: 600 }}>📅 Scheduled</span>}
                      </div>
                      ...
                      <div style={{ fontSize: 14, color: COLORS.textLight, marginBottom: 12 }}>{o.items.map(i => `${i.qty}x ${i.name}`).join(", ")}</div>
                      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                        ...
                        {isCancellable && <button onClick={() => cancelOrder(o.id)} style={{ padding: "13px 20px", background: 'transparent', color: COLORS.error, border: `2px solid ${COLORS.error}`, borderRadius: 14, fontWeight: 800, cursor: 'pointer' }}>❌ Cancel</button>}
                      </div>
                    </div>
                  )
                })}
...
export default function App() {
  ...
  const handlePinSubmit = () => { const aPin = settings?.adminPin || "9876"; const sPin = settings?.staffPin || "5432"; if (targetRole === "admin" && pinInput === aPin) { setRole("admin"); setShowPinModal(false); setPinInput(""); } else if (targetRole === "staff" && (pinInput === sPin || pinInput === aPin)) { setRole("staff"); setShowPinModal(false); setPinInput(""); } else if (targetRole === "customer") { setRole("customer"); setShowPinModal(false); setPinInput(""); } else { alert("❌ Incorrect PIN!"); setPinInput(""); } };
    ...
    const fetchAllData = async () => {
      try {
        ...
        menuSnap.forEach(docSnap => {
          ...
          if (docSnap.id === "appSettings") { setSettings(prev => ({ ...prev, ...data })); }
        });
      } catch (e) { console.error("Firebase fetch error:", e); } finally { setLoading(false); }
    };
    fetchAllData();
  ...
  const deleteBooking = async (id) => { if(window.confirm("Delete this booking?")) { try { await deleteDoc(doc(db, "bookings", id)); } catch(e){} setBookings(bookings.filter(b=>b.id!==id)); } };
  const addInventory = async (item) => { try { await setDoc(doc(db, "inventory", item.id), item); } catch(e){} setInventory([...inventory, item]); };
  const updateStock = async (id, newStock) => { try { await updateDoc(doc(db, "inventory", id), { stock: newStock }); } catch(e){} setInventory(inventory.map(i=>i.id===id?{...i,stock:newStock}:i)); };
  ...
  const placeOrder = async (order, rewardCost = 0) => {
    ...
    if(order.customer.phone && order.customer.phone.length >= 10) {
      ...
      if(existingUserIndex >= 0) { try { await setDoc(doc(db, "loyaltyUsers", order.customer.phone), updatedUsers[existingUserIndex]); } catch(e){} }
      else { try { await setDoc(doc(db, "loyaltyUsers", order.customer.phone), { phone: order.customer.phone, name: order.customer.name, coins: netCoins }); } catch(e){} }
  ...
  const markPaid = async (orderId, paid) => { try { await updateDoc(doc(db, "orders", orderId), { paid }); } catch(e){} setOrdersState(orders.map(o=>o.id===orderId?{...o,paid}:o)); };
